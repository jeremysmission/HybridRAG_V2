...
3. **Embedding must be local** — nomic-embed-text runs on GPU, not available as a cloud API. But embedding is the fastest step (60-200 chunks/sec on NVIDIA workstation GPU), so this is not a bottleneck
