...
Covers:
  ...
  - System alias resolution (monitoring system, legacy monitoring system)
...
@pytest.mark.parametrize("q,expected", [
    ("top failing parts in monitoring system in 2024",            (2024, 2024)),
...
def test_parse_year_range(q, expected):
    assert parse_year_range(q) == expected
...
@pytest.mark.parametrize("text,expected", [
    ("monitoring system report Q1",                 "monitoring system"),
    ("E:\\CorpusTransfr\\monitoring system\\foo",   "monitoring system"),
    ("legacy monitoring system calibration",                 "legacy monitoring system"),
    ("random file",                      ""),
])
...
@pytest.mark.parametrize("q,expected", [
    ("What were the highest failing part numbers in the monitoring systems in 2024?",   True),
    ("Top 5 failing parts in legacy monitoring system in Djibouti from 2022-2025",                     True),
...
def test_alias_tables_load(tmp_path):
    ...
    assert tables.resolve_system("top failing parts in monitoring system in 2024") == "monitoring system"
    assert tables.resolve_system("legacy monitoring system calibration")                    == "legacy monitoring system"
...
def seeded_store(tmp_path):
    ...
    # Seed 15 failure events across monitoring system / legacy monitoring system, 3 sites, years 2022-2025
    events = [
        # monitoring system 2024 Djibouti
        FailureEvent(source_path="p1", chunk_id="c1", part_number="ARC-4471", system="monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-100"),
        FailureEvent(source_path="p2", chunk_id="c2", part_number="ARC-4471", system="monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-101"),
        FailureEvent(source_path="p3", chunk_id="c3", part_number="ARC-4471", system="monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-102"),
        FailureEvent(source_path="p4", chunk_id="c4", part_number="WR-200",  system="monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-103"),
        FailureEvent(source_path="p5", chunk_id="c5", part_number="WR-200",  system="monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-104"),
        FailureEvent(source_path="p6", chunk_id="c6", part_number="AB-115",  system="monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-105"),
        # monitoring system 2023 Guam
        FailureEvent(source_path="p7", chunk_id="c7", part_number="ARC-4471", system="monitoring system", site_token="guam", event_year=2023, incident_id="IGSI-200"),
        FailureEvent(source_path="p8", chunk_id="c8", part_number="FM-440",  system="monitoring system", site_token="guam", event_year=2023, incident_id="IGSI-201"),
        # legacy monitoring system 2024 Djibouti
        FailureEvent(source_path="p9",  chunk_id="c9",  part_number="SEMS3D-5501", system="legacy monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-300"),
        FailureEvent(source_path="p10", chunk_id="c10", part_number="SEMS3D-5501", system="legacy monitoring system", site_token="djibouti", event_year=2024, incident_id="IGSI-301"),
        FailureEvent(source_path="p11", chunk_id="c11", part_number="PS-909",      system="legacy monitoring system", site_token="djibouti", event_year=2023, incident_id="IGSI-302"),
        # legacy monitoring system 2025 Djibouti
        FailureEvent(source_path="p12", chunk_id="c12", part_number="SEMS3D-5501", system="legacy monitoring system", site_token="djibouti", event_year=2025, incident_id="IGSI-400"),
        # legacy monitoring system 2022 Djibouti
        FailureEvent(source_path="p13", chunk_id="c13", part_number="PS-909",      system="legacy monitoring system", site_token="djibouti", event_year=2022, incident_id="IGSI-500"),
        # Unrelated
        FailureEvent(source_path="p14", chunk_id="c14", part_number="AH-777", system="monitoring system", site_token="thule",    event_year=2019, incident_id="IGSI-600"),
        FailureEvent(source_path="p15", chunk_id="c15", part_number="AH-777", system="monitoring system", site_token="thule",    event_year=2020, incident_id="IGSI-601"),
    ]
...
def test_store_coverage_summary(seeded_store):
    ...
    assert cov["distinct_systems"] == 2     # monitoring system, legacy monitoring system


def test_top_n_parts_nexion_2024(seeded_store):
    rows = seeded_store.top_n_parts(system="monitoring system", year_from=2024, year_to=2024, limit=5)
...
def test_top_n_parts_isto_djibouti_2022_2025(seeded_store):
    rows = seeded_store.top_n_parts(
        system="legacy monitoring system", site_token="djibouti",
...
def test_top_n_parts_per_year_nexion(seeded_store):
    per_year = seeded_store.top_n_parts_per_year(
        system="monitoring system", year_from=2019, year_to=2025, limit_per_year=3,
    )
...
def test_evidence_for_part(seeded_store):
    ev = seeded_store.evidence_for_part(
        "ARC-4471", system="monitoring system", year_from=2024, year_to=2024, limit=5,
    )
    assert len(ev) == 3
...
def test_executor_question_1_nexion_2024(seeded_store):
    ...
    assert result.parsed_params["system"] == "monitoring system"
...
def test_executor_question_2_isto_djibouti_range(seeded_store):
    ...
    exec_ = AggregationExecutor(seeded_store, aliases)
    result = exec_.try_execute(
        "What were the highest failing part numbers in the legacy monitoring systems in Djibouti from 2022-2025?"
    )
    assert result is not None
    assert result.tier == "GREEN"
    assert result.parsed_params["system"] == "legacy monitoring system"
...
def test_executor_unsupported_returns_red(tmp_path):
    ...
    result = exec_.try_execute("top 5 failing parts in monitoring system in 2024")
