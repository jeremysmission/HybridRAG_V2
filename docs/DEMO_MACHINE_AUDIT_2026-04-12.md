...
- **Beast** in coordinator language maps to the current dual-RTX-3090 `C:\HybridRAG_V2` box documented elsewhere as the **primary workstation**.
...
## Required Pre-Demo Checks For Beast

Before freezing Beast as the live demo box, all of the following must be true on the same day as the demo or rehearsal:
...
1. **Repo freeze mismatch on Beast:** the current authoritative machine is on `22b22c2` but the tree is dirty. That is acceptable for engineering work, not for demo freeze.
...
- **Authoritative demo machine right now:** **Beast / primary workstation**
