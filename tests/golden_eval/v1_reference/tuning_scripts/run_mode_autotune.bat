...
if exist "%REPO_ROOT%\.venv\Scripts\python.exe" (
    "%REPO_ROOT%\.venv\Scripts\python.exe" tools\autotune\run_mode_autotune.py %*
...
where py >nul 2>nul
if !ERRORLEVEL! EQU 0 (
    py -3 tools\autotune\run_mode_autotune.py %*
    set "EXIT_CODE=!ERRORLEVEL!"
    popd >nul
    exit /b !EXIT_CODE!
)
...
if !ERRORLEVEL! EQU 0 (
    python tools\autotune\run_mode_autotune.py %*
    set "EXIT_CODE=!ERRORLEVEL!"
    popd >nul
    exit /b !EXIT_CODE!
)
