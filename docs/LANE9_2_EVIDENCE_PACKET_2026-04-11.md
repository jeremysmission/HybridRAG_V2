...
5. `settings_guidance_wording`
   Confirms the Settings panel guidance points operators to `config/config.yaml` and restart-based config changes.
...
| `settings_guidance_wording` | PASS | JSON details show required `config/config.yaml and restarting.` phrase present |
