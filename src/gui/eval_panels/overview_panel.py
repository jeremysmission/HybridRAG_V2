...
_LOCAL_ONLY_ROOT = Path(r"C:\Users\jerem\HYBRIDRAG_LOCAL_ONLY")
...
def _hardtail_timing(comparator_manifest_path: Path) -> dict:
    ...
    # Claude shape: comparator manifest has no timings; fall back to the
...
def _section_hardtail() -> list[str]:
    ...
    claude = _read_json(CLAUDE_TRAINING40_MANIFEST) or {}
    codex = _read_json(CODEX_TRAINING40_MANIFEST) or {}
    ...
    if claude or codex:
        ...
        for label, m, manifest_path in (
            ("Claude Max", claude, CLAUDE_TRAINING40_MANIFEST),
            ("Codex", codex, CODEX_TRAINING40_MANIFEST),
        ):
            if not m:
...
def _section_count_benchmark() -> list[str]:
    ...
    lines.append(
        "- Status: Ready for QA (Codex-Researcher shipped 2026-04-15: pytest 8 passed, live run 7/7 frozen-expectation verification)"
    )
    lines.append("")
    return lines
...
def _section_aggregation() -> list[str]:
    ...
    lines.append(
        "- Status: Ready for QA (Agent1 validated-on-disk 2026-04-15: pytest 9 passed, runner Gate PASS 12/12)"
    )
    lines.append("")
    return lines
...
def render_overview_markdown() -> str:
    ...
    footer = [
        "---",
        "",
        "Signed: Claude-Max - HybridRAG_Educational / HYBRIDRAG_LOCAL_ONLY - 2026-04-15 MDT",
        "",
    ]
