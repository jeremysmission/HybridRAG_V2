...
|---|---|---|
| G0 | Installer correctness is frozen: Agent 1 B+C lands, `scripts\verify_install.py` passes on the workstation desktop and laptop | Any unattended workstation run |
| G1 | Tier 2 OOM root cause is confirmed and the recovery action is approved | Clean Beast work resumes |
...
- **After G1:** Run the clean Tier 1 rerun on the authoritative Beast store.
...
- **Preferred use after G0:** Long unattended rebuild or extraction work that should not block Beast.
...
- Close **G1** on Beast first. Without the Tier 2 resolution, the clean store cannot be trusted.
- Close **G0** in parallel on the workstation desktop and laptop. Do not wait on Beast to freeze installer correctness.
- Keep Beast GPU 0 open for fast checks while GPU 1 stays protected.

### Phase 2 — Rebuild and remeasure

- Once **G1** closes, run the clean Tier 1 rerun on Beast GPU 1 and freeze the resulting counts.
...
- **Primary live path:** retrieval-first demo on the authoritative Beast store
